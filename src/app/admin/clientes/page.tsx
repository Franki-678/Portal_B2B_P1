          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="mb-2 flex items-center gap-2">
                  <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-800 bg-slate-950 text-xl">
                    🏭
                  </div>
                  <div>
                    <h2 className="font-semibold text-slate-100">{workshop.name}</h2>
                    <p className="text-xs text-slate-500">{workshop.contact_name || 'Sin contacto'}</p>
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-right">
                <div className="text-[10px] uppercase tracking-[0.18em] text-slate-500">Alta</div>
                <div className="text-xs text-slate-300">{formatDate(workshop.created_at)}</div>
              </div>
            </div>
            ...
            {workshop.lastOrder && (
              <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-xs text-slate-400">
                Ultimo pedido: <span className="font-medium text-slate-200">{workshop.lastOrder.items?.[0]?.partName || 'Sin repuestos'}</span>
              </div>
            )}
          </div>
        ))}
