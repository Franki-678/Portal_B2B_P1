...
// ─── ORDER TABLE ROW (for vendor / taller table view) ─────

interface OrderRowProps {
  order: Order;
  onClick?: () => void;
  role?: 'taller' | 'vendedor';
  /** Slot de acciones (botón Eliminar, etc.) — se renderiza en la última columna */
  actions?: ReactNode;
}

export function OrderTableRow({ order, onClick, role, actions }: OrderRowProps) {
  const firstItem = order.items?.[0];
  const itemsCount = order.items?.length ?? 0;
  const partNameDisplay = itemsCount > 1
    ? `${firstItem?.partName} (+${itemsCount - 1})`
  ...
  return (
    ...
    >
      ...
      <td className="px-5 py-4">
        <span className="text-xs font-mono font-bold text-zinc-300 bg-zinc-900/50 px-2 py-1 border border-zinc-800/80 rounded group-hover:text-zinc-100 transition-colors uppercase whitespace-nowrap">
          {role === 'vendedor' && order.workshop?.tallerNumber && order.workshopOrderNumber
            ? `${String(order.workshop.tallerNumber).padStart(2, '0')}-PED-${String(order.workshopOrderNumber).padStart(4, '0')}`
            : order.workshopOrderNumber
              ? `PED-${String(order.workshopOrderNumber).padStart(4, '0')}`
      ...
      <td className="px-5 py-4">
        ...
        <div className="text-xs text-zinc-500 font-medium mt-0.5">
          {order.vehicleBrand} {order.vehicleModel} <span className="text-zinc-600">· {order.vehicleYear}</span>
        </div>
      </td>

      {/* Columna: Taller — solo para vendedor/admin */}
      {role !== 'taller' && (
        <td className="px-5 py-4">
          {order.workshop && (
